...
mtestts = window(mgts, start = c(2015, 1))
```
...
Comments:
Adjusted R squared shows that GDP explains 81% of the variability in sales, implying a relatively good fit.
Residual standard error of 742.2 is reasonably close to 711 standard error, also implying relatively good fit.
...
Comment: Residuals appear to have a slight cyclical trend and may not be completely random although trend is not prominent. The  outliers are once again notable.
...
par(mfrow = c(1,1))
```
Comment: Q-Q plot shows moderate skew. However, resdiduals tend to cluster around line with the exception of a number of notable outliers. 
